...
function Header(props) {
  ...
  const seacrhInput = Object(react__WEBPACK_IMPORTED_MODULE_0__["useRef"])(null); // handle what happens on key press

  const handleKeyPress = Object(react__WEBPACK_IMPORTED_MODULE_0__["useCallback"])(event => {
    if (event.key === 'Meta' || event.key === 'Control' || event.key === 'k') setTyped(true);

    if (event.key === 'k' && metaTyped) {
      console.log(seacrhInput.current);
      setTyped(false);
    }

    if (!metaTyped) {
      if (event.key !== 'k' && event.key !== 'Control' && event.key !== 'Meta') setTyped(false);
    }

    console.log(event.key, metaTyped);
  }, []);
...
//# sourceMappingURL=main.b6d15df877453b23b61d.hot-update.js.map