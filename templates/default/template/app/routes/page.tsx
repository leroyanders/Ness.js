...
/**
 * What `page.server.ts`'s loader returns. Declared here rather than inferred
 * from it: importing the server module into a component is exactly what the
 * `.server` convention exists to prevent, so the shape is stated on both
 * sides instead of shared.
 */
interface PageData {
  renderedAt: string;
  node: string;
...
const time = (value: string): string => new Date(value).toLocaleTimeString();
...
function useApiHealth(): ApiHealth {
  const [state, setState] = useState<ApiHealth>({ status: 'pending' });
...
function apiRow(health: ApiHealth): { badge: ReactNode; note: string } {
  ...
  return (
    <div className="page">
      ...
      <div className="shell">
        ...
        <section className="trace" aria-label="Request trace">
          ...
          <dl>
            ...
            <div className="row">
              <dt>API</dt>
              <dd>
                app/server/api.controller.ts {api.badge}
        ...
        <p className="next">
          Edit <code>app/routes/page.tsx</code> and save. The page updates
          without a reload.
        </p>
