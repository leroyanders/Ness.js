...
Ness serves each locale from its own URL prefix. Routes are written once; the router mounts that one tree again under each locale's own path segment.
...
const localize = createLocaleMiddleware(i18n);

export default function localeMiddleware({ request }) {
  return localize(request);
}
```

`createLocaleMiddleware` returns a plain `(request) => Response | undefined`, so it is called with the `request` out of the middleware arguments rather than exported directly. Returning the redirect ends the request there; returning `undefined` lets it continue to the route.
