...
test('package exposes the ness binary', () => {
  assert.deepEqual(packageJson.bin, { ness: './dist/bin/index.js' });
});

test('CLI exposes the new application command', () => {
  const result = spawnSync(
    process.execPath,
    [path.join(currentDirectory, '..', 'dist', 'bin', 'index.js'), '--help'],
    {
      encoding: 'utf8',
    },
  );
  ...
  const generateHelp = spawnSync(
    process.execPath,
    [
      path.join(currentDirectory, '..', 'dist', 'bin', 'index.js'),
      'generate',
      '--help',
    ],
    { encoding: 'utf8' },
...
test('official template sources use one Ness configuration file', t => {
  ...
  for (const name of ['default', 'typescript', 'minimal', 'api', 'dashboard']) {
    ...
    assert.equal(
      fs.existsSync(path.join(destination, 'ness.config.ts')),
      true,
      `${name} must include ness.config.ts`,
    );
    for (const legacy of [
      '.npmignore',
      '.prettierrc',
      'instrumentation.mjs',
      // The templates moved to a TypeScript config; a leftover .mjs beside it
      // would be resolved first by nothing and read by no one.
      'ness.config.mjs',
