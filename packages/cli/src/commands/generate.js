...
export function resolveGeneratorType(rawType) {
...
function projectUsesTypeScript(cwd = process.cwd()) {
  return fs.existsSync(path.join(cwd, 'tsconfig.json'));
}

function safeGeneratorName(name) {
...
function generatorSource(type, name, typescript) {
...
function generatorFilename(type, name, cwd, typescript) {
...
function registerNestProvider(type, name, filename, cwd, dryRun) {
  const registrations = {
  ...
  const insertion = imports.length
    ? imports.at(-1).index + imports.at(-1)[0].length
    : 0;
  ...
  if (fieldPattern.test(source)) {
    source = source.replace(fieldPattern, (_match, start, values, end) => {
