...
/** Something the migration did, or something it refuses to guess at. */
export interface MigrationFinding {
  file: string;
  level: 'manual' | 'rewritten';
  message: string;
}
...
interface ImportRewrite {
  from: string;
  /** `null` when there is no equivalent module to point at. */
  to: string | null;
...
function gitIsClean(root: string): boolean | undefined {
  try {
...
function rewriteImports(
  ...
  for (const rule of IMPORT_REWRITES) {
    ...
    if (rule.to === null) {
      // Every `to: null` rule carries the note that replaces the rewrite.
    ...
    if (defaultName) {
      ...
      output = output.replace(
        defaultImport,
        (_match: string, local: string, quote: string) => {
          rewroteClause = true;
          const clause =
...
function describeRouteFile(
  file: string,
  appDirectory: string,
): { relative: string; segments: string[]; name: string; extension: string } {
 ...
 * `app/root.tsx`.
 */
function planMigration(root: string): MigrationPlan {
...
function migrateNextConfig(root: string, findings: MigrationFinding[]): void {
...
function renderReport(
  root: string,
  { moves, findings }: Pick<MigrationPlan, 'moves' | 'findings'>,
): string {
  const byLevel = (level: MigrationFinding['level']) =>
    findings.filter(finding => finding.level === level);
...
export async function migrateFromNext(
  directory = '.',
  options: MigrateOptions = {},
): Promise<MigrationPlan & { report: string }> {
