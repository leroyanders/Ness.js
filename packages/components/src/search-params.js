 ...
 */
function useSearchParam(name, { defaultValue = '', replace = true } = {}) {
 ...
 */
function useSubmitOnChange({ replace = true } = {}) {
  const submit = useSubmit();
  return useCallback(
    event => submit(event.currentTarget, { replace, preventScrollReset: true }),
    [replace, submit],
  );
}
