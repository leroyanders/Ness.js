 ...
 * hunting for it.
 */
function Meta({ children }: MetaProps): ReactNode {
 ...
 */
function Title({ children }: TitleProps): ReactNode {
...
function Description({ children }: DescriptionProps): ReactNode {
 ...
 */
function Canonical({ href }: CanonicalProps): ReactNode {
  return createElement('link', { rel: 'canonical', href });
}

/** Crawler instructions, for example `noindex, nofollow`. */
function Robots({ children }: RobotsProps): ReactNode {
 ...
 * small thumbnail.
 */
function SocialImage({ src, alt }: SocialImageProps): ReactNode {
  return createElement(
    Fragment,
    null,
