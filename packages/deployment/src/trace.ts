...
/** A second version of a name, and where it has to be nested to resolve. */
...
function readManifest(directory: string): PackageManifest | undefined {
  ...
  try {
    return JSON.parse(fs.readFileSync(filename, 'utf8')) as PackageManifest;
  } catch {
    return undefined;
  }
}
...
function traceDependencies(
  root: string,
  { manifest = readManifest(root), extra = [], onMissing }: TraceOptions = {},
): TraceResult {
