...
test('the generated entry imports the runtime config when there is one', () => {
  ...
  assert.match(withConfig, /createVercelHandler\(\{ build, root, config \}\)/);
});
...
test('createVercelOutput fails clearly when there is no build', async t => {
  ...
  await assert.rejects(
    createVercelOutput({ root, logger: null }),
    /ness build/,
  );
});
...
test('createVercelOutput produces a Build Output API v3 tree', async t => {
  ...
  assert.ok(
    fs.existsSync(path.join(functionDirectory, 'build', 'server', 'index.js')),
    'server build is copied',
  );
  assert.ok(
    fs.existsSync(
      path.join(functionDirectory, 'build', 'nest', 'app.module.js'),
    ),
  ...
  assert.ok(
    fs.existsSync(
      path.join(functionDirectory, 'node_modules', 'ws', 'package.json'),
    ),
  ...
  assert.ok(
    fs.existsSync(path.join(root, '.vercel', 'output', 'static', 'index.html')),
...
test('createVercelOutput bundles a runtime config when one is on disk', async t => {
  ...
  const entry = fs.readFileSync(path.join(report.function, 'index.js'), 'utf8');
  assert.match(
    entry,
    /import \* as config from '\.\/ness\.server\.config\.mjs'/,
  );
