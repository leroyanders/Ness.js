...
import { decodeEntry, encodeEntry } from '../dist/adapters/serialize.js';
import { NessCache } from '../dist/index.js';
...
test('binary nested inside an object survives, as a page body is', () => {
  const back = roundTrip({
    ...
    status: 200,
  }) as { body: unknown; headers: string[][]; status: number };
...
test('binary survives the filesystem adapter end to end', async () => {
  ...
  const read = await cache.read<{ body: unknown; status: number }>('page:/');
