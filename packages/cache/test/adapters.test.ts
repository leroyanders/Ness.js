...
import { FileSystemCacheAdapter } from '../dist/adapters/filesystem.js';
import { RedisCacheAdapter } from '../dist/adapters/redis.js';
import {
  TieredCacheAdapter,
  createRedisInvalidationBus,
} from '../dist/adapters/tiered.js';
import type { InvalidationMessage } from '../dist/adapters/tiered.js';

/** The fixtures below all back their indexes, so the tests may rely on them. */
type IndexedAdapter = CacheAdapter &
  Required<Pick<CacheAdapter, 'keysByTag' | 'keysByPath'>>;
...
const ADAPTERS: Array<[string, () => Promise<AdapterFixture>]> = [
...
test('an invalidation bus evicts local copies on other instances', async () => {
  const shared = new MemoryCacheAdapter();
  const handlers: Array<(message: InvalidationMessage) => void> = [];
  const bus = () => ({
    publish: (message: InvalidationMessage) => {
...
test('the redis invalidation bus ignores messages it published itself', async () => {
  ...
  const client = {
    ...
    subscribe: (_channel: string, listener: (payload: string) => void) => {
...
test('concurrent writes of the same key do not collide on a temp file', async t => {
  ...
  assert.ok(
    ['a', 'b', 'c'].includes(stored!.value as string),
    'one writer wins cleanly',
  );
  assert.deepEqual(
    fs
...
test('an index entry that no longer carries the tag is not deleted', async () => {
  ...
  adapter.keysByTag = async (tag: string) =>
...
test('the redis bus tolerates an ioredis subscriber', () => {
  ...
  const subscriber = {
    subscribe: (
      _channel: string,
      done?: (error: Error | null, count: number) => void,
    ) => {
      done?.(null, 1);
