...
/** Encodings we can produce, best first. */
 ...
 *
 * Honours quality values, including `q=0`, which is how a client says "not
 ...
 */
function negotiateEncoding(header, available = ENCODINGS) {
 ...
 * A 204 or a 304 has no body to work on.
 */
function shouldCompress(response, { threshold = DEFAULT_THRESHOLD } = {}) {
  ...
  if (length !== null && Number(length) < threshold) return false;
 ...
 */
function compressResponse(request, response, options = {}) {
  ...
  const body = Readable.toWeb(
    Readable.fromWeb(response.body).pipe(compressor(encoding)),
  );
