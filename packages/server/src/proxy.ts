export interface ForwardedOptions {
  /**
   * Honor `X-Forwarded-Proto` and `X-Forwarded-Host`.
   *
...
/** Schemes worth honoring. Anything else is a client making things up. */
 ...
 * generated link.
 */
function forwardedHost(headers: Headers): string | undefined {
...
function applyForwardedHeaders(
  request: Request,
  { trustProxy = false }: ForwardedOptions = {},
): Request {
  ...
  if (host) {
    ...
    // honored; the absence of one means the scheme's default.
  ...
  return new Request(url, {
    ...
    // Node requires this whenever a body is a stream, and it is not part of
