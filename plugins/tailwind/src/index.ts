...
type PostcssPlugins =
  NonNullable<
    NonNullable<import('vite').UserConfig['css']>['postcss']
  > extends { plugins?: infer P }
    ? NonNullable<P>
    : never;

function postcssPlugins(production: boolean): PostcssPlugins {
  return [
    tailwindcss(),
...
function tailwind(options: TailwindOptions = {}): Plugin {
  return {
    name: 'ness:tailwind',
    enforce: 'pre',
