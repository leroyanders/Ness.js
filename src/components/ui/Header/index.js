...
function Header(props) {
    ...
    // save shemaMode 
    const [mode, setMode] = useLocalStorage('schema-mode', 'system');
    ...
    const setShemaMode = (schema) => {
        setMode(schema);
        if (schema !== 'system') {
            (schema === 'dark'? 'dark' : 'light') === 'dark'? document.body.classList.add('dark') : document.body.classList.remove('dark');
        } else {
            toogleSchema('system');
        }
    }

    const toogleSchema = (schema) => {
        if(schema === 'system') { 
            return (window.matchMedia('(prefers-color-scheme: dark)').matches? 'dark' : 'light') === 'dark'? document.body.classList.add('dark') : document.body.classList.remove('dark');
        }

        if(schema === 'dark') {
            mode === 'system'? setMode('system') : setMode('dark');
            document.body.classList.add('dark')
        } else {
            mode === 'system'? setMode('system') : setMode('light');
    ...
    useEffect(() => {
        ...
        window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', event => {
            if (mode === 'system') return toogleSchema(window.matchMedia('(prefers-color-scheme: dark)').matches? 'dark' : 'light');
        ...
        toogleSchema(mode === 'system'? (window.matchMedia('(prefers-color-scheme: dark)').matches? 'dark' : 'light') : mode);
                        ...
                        <nav className="mt-1.5 ml-auto">
                            <div className="flex">
                                <nav className="mr-5 flex">
                                    ...
                                    <DropdownMenu mode={mode} setMode={setShemaMode} />
                                </nav>
                                <form>   
